...
- **A GitHub runner is ~2.1× slower than a 2-core local container** (measured: `adaptive_strategy_test` 224s local vs 478s on CI). Use that factor before adding any `duration.as_secs() < N` assertion — the surviving ones have 6×+ headroom, which is why they don't flake. A 2-core local cap alone does **not** reproduce CI timing.
