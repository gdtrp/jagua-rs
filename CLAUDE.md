...
make test-slow  # + the `slow-tests` feature: the minutes-long LBF tests (when you touched packing)
```
...
- `cutl195_mixed_irregular.rs` replays the staging request behind CUTL-195 (rounded rect + circle) and pins the group-mode contract: each type's full sheets are byte-identical copies of its single-type stencil, only leftovers share sheets. Fixtures live in `jagua-sqs-processor/tests/testdata/cutl195/`.
...
- **Slow tests are gated behind the `slow-tests` cargo feature**, declared in both owned crates: `#[cfg_attr(not(feature = "slow-tests"), ignore = "slow: …")]`. A plain `cargo test` / `make test` / the CI `test` job skips them (they show up as "ignored"); `make test-slow` and the nightly `slow` CI job (also `workflow_dispatch`) run them. Gate a test when it does real LBF packing for more than ~10 s alone — on the 2-core runner that is 60 s+, and before gating, the `test` job took 27 min (13 of them the processor's `e2e_test.rs`). Gated today: five max-fit / middle-page tests in `adaptive_strategy_test.rs`, `production_cases_report` in `cutl160_prod.rs`, and the LBF-heavy cases in the processor's `e2e_test.rs`. When you change packing behaviour, run `make test-slow` yourself; CI will only tell you the next morning.
- **The `integration` CI job and `make test-integration` run only `kafka_integration_test` and `s3_endpoint_integration_test`.** A bare `-- --ignored` over the crate would also run every other `#[ignore]`d test, i.e. the wall-clock e2e test and the whole slow suite. MinIO images are pulled from `quay.io/minio/*`: the Docker Hub `minio/minio` repository stopped resolving on 2026-09-16 and took the job down with "pull access denied".
- **Wall-clock tests are `#[ignore]`d, not deleted.** `test_e2e_processing_dr_svg` (processor) and five tests in `jagua-utils/tests/adaptive_strategy_test.rs` assert throughput, not correctness — they fail on a slow or loaded machine and pass on a developer box. Those two kept the CI `test` job red on every `vk-cloud` push from the day CI was introduced. Run them with `-- --ignored`; don't let one gate a merge.
- **Don't add `duration.as_secs() < N` assertions to nesting tests, and don't trust an isolated measurement of one.** libtest runs a file's tests concurrently and the strategy uses rayon internally, so on a 2-core runner they starve each other: `test_rounded_rect_with_holes_packs_21_parts` takes ~9s alone and took **152s** inside the full suite (~17×). Whole-binary numbers are the only meaningful ones (`adaptive_strategy_test`: 224s local vs 478s on CI). Three separate red CI cycles came from bounds like this. The one that survives is gated behind `JAGUA_PERF_ASSERTS=1` — set it to enforce wall-clock bounds locally.
